...
echo -e ""
echo -e "${GREEN}B) systemd (ajánlott, újraindul crash után):${NC}"
echo -e "   ${YELLOW}sudo cp $BOT_DIR/deploy/sebessegbot.service /etc/systemd/system/${NC}"
echo -e "   Ellenőrizd a fájlban: User, WorkingDirectory, ExecStart (HOME útvonalak)."
echo -e "   ${YELLOW}sudo systemctl daemon-reload && sudo systemctl enable --now sebessegbot${NC}"
echo -e "   Állapot: ${YELLOW}sudo systemctl status sebessegbot${NC}  |  log: ${YELLOW}journalctl -u sebessegbot -f${NC}"
echo -e "=========================================================="
