...
impl StrategyConfig {
    pub fn notional_per_level_usd(&self, equity_usd: f64) -> f64 {
        let computed = equity_usd * (self.balance_pct_per_trade / 100.0) * (self.leverage as f64);
...
impl AppConfig {
    pub fn load() -> Self {
        ...
        let active_coin = env::var("ACTIVE_COIN")
        ...
        if config_str.is_empty() {
            panic!("❌ strategy_maker.json nem talalhato!");
        }

        let parsed: Value = serde_json::from_str(&config_str)
            .unwrap_or_else(|e| panic!("❌ Config JSON parse hiba: {}", e));
        ...
        let mut strategy = StrategyConfig {
            ...
            base_sz_usd: rm["position_limits"]["max_notional_usd_per_trade"]
                .as_f64().unwrap_or(50.0),
            balance_pct_per_trade: rm["position_limits"]["balance_pct_per_trade"]
                .as_f64().unwrap_or(2.0),
            max_positions: rm["position_limits"]["max_open_positions"]
            ...
            min_signal_interval_ms: parsed["signal_engine"]["debounce"]["min_time_between_entries_ms"]
                .as_u64().unwrap_or(15000),
            max_daily_loss_usd: rm["daily_limits"]["max_daily_loss_usd"]
                .as_f64().unwrap_or(20.0),
            max_daily_trades: rm["daily_limits"]["max_daily_trades"]
                .as_u64().unwrap_or(30) as u32,
            tp_min_ticks: om["exit"]["take_profit"]["min_profit_ticks"]
                .as_f64().unwrap_or(35.0).max(15.0),
            sl_min_ticks: om["exit"]["take_profit"]["min_sl_ticks"]
                .as_f64().unwrap_or(20.0).max(10.0),
            maker_fee_rate: parsed["fees"]["maker_fee_rate"]
                .as_f64().unwrap_or(0.00015),
            taker_fee_rate: parsed["fees"]["taker_fee_rate"]
                .as_f64().unwrap_or(0.00045),
        };
